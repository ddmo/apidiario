...
/** Contenuto personalizzazione vista Express — riusato dalla route /impostazioni/ispezione-express e dal pannello Account desktop/tablet. */
export function ExpressSettingsContent({ hideHeader }: { hideHeader?: boolean }) {
  ...
  async function handleSave() {
    try {
      ...
      if (!hideHeader) router.history.back()
    } catch {
  ...
  return (
    <div className={hideHeader ? 'flex flex-col h-full' : 'fixed inset-0 tablet:relative tablet:inset-auto bg-cream-50 text-wood-700 flex flex-col z-10 tablet:z-auto tablet:h-full'}>
      {!hideHeader && (
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-5 xl:px-8" style={{ paddingBottom: 'calc(1rem + 3.5rem)' }}>
        <div className="flex flex-col gap-1 xl:max-w-4xl xl:mx-auto xl:block xl:columns-2 xl:gap-x-10 [&>div]:xl:break-inside-avoid [&>div]:xl:mb-1">
